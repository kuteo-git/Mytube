...
3. **`untranslated.guard.test.ts`** — a source scan, and **the only layer that can see the fault the request was about.** A string that was never extracted is not a missing key; it is a literal in a component that renders in English while nothing reports it. Proven by putting one back and watching the build fail with `pages/HistoryPage.tsx:15 [jsx-text] Watch history`.
   ...
   - **Template literals are scanned too, and that was the second hole.** The rule read `'` and `"` and not backticks — which is exactly where copy goes the moment it interpolates anything, and interpolating copy is a *sentence* rather than a label. Eleven were live while every check was green: "View more (3)", "Playing from …", "Writable. 235 GB free, 914 videos already there.", and two toasts carrying `job${count !== 1 ? 's' : ''}` — an English plural rule, in a language with none.
...
### Speech is a URL, and the protocol is OpenAI's (2026-08-22)

**The synthesiser used to be `http://localhost:8002/tts`, written into `tts.go`, speaking a shape of its own.** That is fine for one machine and it is the whole obstacle for anybody else: the service lives in a *different repository* (`robot-esp32/services/vieneu_server.py`, reached through `$HOME`), so somebody cloning this one gets `narration will be silent` and has nothing to point at instead.

- **VieNeu learned the protocol; the app did not learn VieNeu's.** `POST /v1/audio/speech` was added to that server beside the `/tts` it already had, so this app speaks exactly one dialect and the endpoint is a setting. Anyone with an OpenAI key — or any of the many services that copy its shape — types a URL and it works. The reverse (a provider field, one code path per provider) was proposed and rejected: it is two things for a stranger to configure and two paths to keep alive.
- **`emotion: "storytelling"` became `instructions`**, which is OpenAI's own field for how a line should be read. One field, correct at both ends.
- **`response_format` is `wav`, not the mp3 default.** The next thing that happens to these bytes is ffmpeg stretching them into a subtitle slot, so mp3 would mean decoding and re-encoding once for no reason but somebody else's default. VieNeu **refuses** any other format in an OpenAI-shaped error rather than returning wav bytes under an mp3 label — measured: `mp3` → 400 with `invalid_request_error`, `wav` → 200 and 109 KB of audio.
- **The base URL includes `/v1`,** as every provider's documentation gives it, and both conventions are normalised — in the speech field, the translate field and the model listing alike. They used to disagree: translate appended `/v1` itself, so two inputs on one settings screen wanted different things and pasting one into the other gave `/v1/v1/chat/completions` and a 404 that explains nothing. `speechURL`, `providerURL` and `chat_url` each accept a base with `/v1`, without it, or the full endpoint.
- **`thinking: {"type": "disabled"}` is not an OpenAI parameter**, and it is worth keeping: measured on a real 15-cue batch it took the same translation from **5.1s and 649 completion tokens to 2.5s and 345**. Providers that do not know the field answer **400** rather than ignoring it, so the batch is retried once without it — and only on a 400 whose body names the field, because a 400 for any other reason is a real error that a blind retry would hide behind a second identical failure. Verified against a provider that refuses it: two attempts, `thinking` present then absent, translation returned.
- **There is no built-in default, deliberately.** A default that happens to be right on the machine this was written on is how a setting looks configured to everybody who has not configured it, and the first symptom is silence — the hardest fault to trace back to a text field. The cost is real and was accepted: after this change narration is silent here too until the URL is typed in.
- **Unconfigured is not an error.** `errNotConfigured` answers **409**, distinct from the 502 that means "the thing you configured is down": one is fixed on a settings screen and the other by looking at a server. The narration switch is drawn *disabled with a hint naming where to go*, rather than switching on and producing silence — §5's one prohibition, in its worst form, since somebody meeting it checks the volume, the subtitles and the video before an empty text field.
- **The voice is typed, not chosen.** `GET /api/tts/voices` is gone: OpenAI publishes no endpoint listing voices, and every imitator brings its own names, so a menu is right for one provider and wrong the day it adds a voice — the voice exists and the app refuses it. It stays a **per-device** preference; the config's `voice` is only the fallback when a request names none.
- **Test returns the clip, not a verdict.** An endpoint can answer 200 with perfectly formed silence, or a voice reading English phonetics over Vietnamese; a status code and a duration report both as success. It also tests the *form's* values rather than the saved ones — testing after saving is testing what you have already accepted. `synthesiseWith` is the one body and one set of headers both paths use.
- Measured through the running stack: unconfigured → `{"error":"no base url"}` at 200; configured → 244 KB of wav in **308 ms**.
