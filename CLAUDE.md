...
- **Do not exercise a live cookie file by hand.** Both sessions here died within an hour of three manual `yt-dlp --cookies data/cookies/*.txt` runs against the jar the service was using. **Unproven** as cause — it is also simply what Google does — but the cost of being wrong is a dead session and the cost of the precaution is `cp`.
