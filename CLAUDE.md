...
- **The base URL includes `/v1`,** as every provider's documentation gives it, and both conventions are normalised. This project's *own* translate field works the other way round — it appends `/v1` itself — so the two sit on one settings screen disagreeing about what a base URL is. Somebody will paste one into the other.
