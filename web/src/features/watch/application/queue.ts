...
import { useChannelVideos, useTopPlayed } from '@/features/catalog/application/queries'
...
export function useQueue(currentVideoId: string | undefined): Queue {
    ...
    if (channelId) {
      items =
        channelQuery.data?.pages.flatMap((page) =>
          page.videos.map((v) => ({ id: v.id, title: v.title })),
        ) ?? []
    } else if (list === TOP_PLAYED_LIST) {
      items = (topPlayedQuery.data ?? []).map((v) => ({ id: v.id, title: v.title }))
    }
  ...
  }, [channelId, list, sort, currentVideoId, channelQuery.data, topPlayedQuery.data])
}
