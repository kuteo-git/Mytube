...
func NewRanker(store domain.SignalStore, features domain.FeatureSource) *Ranker {
	return &Ranker{store: store, features: features, now: time.Now}
}
...
func (r *Ranker) GetFeed(ctx context.Context, userID, topic string, pageSize, offset int32) ([]domain.RankedVideo, error) {
	...
	return sortAndPage(ranked, pageSize, offset), nil
}
...
// sortAndPage orders by score, breaking ties on video id so that repeated
